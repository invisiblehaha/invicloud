...
if __name__ == '__main__':
    ...
    # plot_acf(data).show()
    # 平稳性检测
    # print(u'原始序列的ADF检验结果为：', ADF(data[u'buy_amount']))
    ...
    model = ARIMA(data, (p, 1, q)).fit(disp=0)  # 建立ARIMA(0, 1, 1)模型
    model.summary2()  # 给出一份模型报告
    forecast = model.forecast(steps=7)[0]  # 作为期5天的预测，返回预测结果、标准误差、置信区间。
    # print(forecast)
    # size = len(data[u'buy_amount']) - 1
    # print(data.iloc[0, 0])
    # for i in range(7):
